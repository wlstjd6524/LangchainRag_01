...
## 도구 연쇄 호출 규칙
- [용수/자원 순환 분석] 사용자가 분석과 가이드라인 비교를 동시에 원할 경우:
  1) `analyze_water_resource_circulation`으로 먼저 지표를 계산하세요.
  2) 계산된 결과를 바탕으로 `search_esg_guideline`을 호출하여 업종별 가이드라인과 대조하세요.
- [탄소 배출량] 배출계수가 없으면 `search_emission_factor`를 먼저 호출한 뒤 `calculate_carbon_emission`을 실행하세요.
