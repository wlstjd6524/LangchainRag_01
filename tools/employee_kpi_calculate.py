...
def _parse_employees(employees_json: str) -> list[Employee] | str:
    """
    JSON 파싱 + Employee 객체 변환 공통 점수
    실패 시 에러 메시지 문자열 반환
    """
    try:
        ...
        return f"직원 데이터 형식 오류: {e}\n필수 필드(id, gender, age, employment_type, position_level)를 확인해주세요."


def _build_tool_input(employees_json: str) -> DiversityInput | str:
    """
    Employee 리스트 → EmployeeKPITool 인스턴스 생성 공통 함수
    실패 시 에러 메시지 문자열 반환
    """
    employees = _parse_employees(employees_json)
    if isinstance(employees, str):  # 에러 메시지인 경우
        return employees
    
    return EmployeeKPITool(
        DiversityInput(company_name="입력사", fiscal_year=2024, employees=employees)
    )


@tool
def calculate_gender_ratio_tool(employees_json: str) -> str:
    """
    직원 데이터를 입력받아 성별 비율 및 K-ESG S-3-1 점수를 계산합니다.
    
    employees_json 형식 예시:
    [
      {
        "id": "001",
        "gender": "M 또는 F",
        "age": 35,
        "employment_type": "regular / part_time / outsource / dispatch 중 하나",
        "position_level": "staff / manager / executive 중 하나",
        "is_board_member": false,
        "is_disabled": false,
        "is_severe_disabled": false
      }
    ]
    """
    tool_instance = _build_tool_input(employees_json)
...
@tool
def calculate_age_ratio_tool(employees_json: str) -> str:
    """
    직원 데이터를 입력받아 연령대별 비율을 계산합니다. (GRI 405-1, 고령자고용촉진법)
...
@tool
def calculate_disability_ratio_tool(employees_json: str) -> str:
    """
    직원 데이터를 입력받아 장애인 고용률 및 K-ESG S-3-3 점수를 계산합니다.
    장애인은 is_disabled=true, 중증장애인은 is_severe_disabled=true로 입력하세요.
...
@tool
def calculate_employment_type_ratio_tool(employees_json: str) -> str:
    """
    직원 데이터를 입력받아 정규직/비정규직 비율 및 K-ESG S-2-2 점수를 계산합니다.
    employment_type: regular(정규직), part_time(단시간), outsource(용역), dispatch(파견)
